...
#### データ取得戦略
- **マスタデータ**: HPB公開サイトからスクレイピングし、Djangoキャッシュに保持
- **スタイリスト情報**: 6時間キャッシュ（キー: `hpb:stylists:{salon_url}`）
- **クーポン情報**: 6時間キャッシュ（キー: `hpb:coupons:{salon_url}`）
  - 直近追加分をすぐ反映したい場合はキャッシュクリアが必要（現実装では自動リフレッシュなし）
