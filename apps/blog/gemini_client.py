...
- 各記事は必ずtitleとcontentを含めてください"""
    ...
    def generate_blog_content(
        ...
        """
        Generate single blog post content using Gemini (legacy method).
        ...
        """
        try:
            system_instruction = """あなたは美容サロンのブログライターです。
...
重要：JSON以外の文字列は出力しないでください"""
            ...
            if result is None:
                ...
                return {
                    ...
                    'model': self.model_id,
                    'error': 'JSON parse error - used fallback',
                }
            ...
            return {
                'title': result.get('title', title or 'Untitled')[:25],
                'content': content,
                'model': self.model_id,
                'prompt_tokens': 0,
                'completion_tokens': 0,
...
"""
            ...
            if response and response.text:
                ...
                return title[:25] if title else current_title
